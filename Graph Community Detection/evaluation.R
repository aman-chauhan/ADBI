...
## Influence Propagation Arguments: Graph = Structural Graph, Igraph Instance Comm
## = Communiity/Clusters Beta = Influence Propagation Rate
...
influence_propagation <- function(graph, comm, beta = 0.75) {
    ...
    cat("Time Step = ", timestep, ", Total Influencers = ", total_influencers, ", Network Coverage = ",
        ratio, "\n")
    ...
    for (i in 1:15) {
        ...
        for (i in influencers) {
            ...
            if (sample(1:100, 1) <= alpha * 100) {
                ...
                candidates = nodes[nodes$comm == nodes[i, c("comm")] & nodes$color ==
                  "green", c("Id")]
                candidates = intersect(candidates, nbs)
                ids <- c()
                for (j in candidates) {
                  if (j %in% nbs && sample(1:100, 1) <= beta * 100) {
                    # Stochastically determining if node will get infected, using beta transmission
                    # probability
                    ids <- append(ids, j)
                  }
                }
        ...
        cat("Time Step = ", timestep, ", Total Influencers = ", total_influencers,
...
plot(op1, type = "b", col = 2, pch = 20, ylim = c(0, vcount(g)), xlim = c(1, max(tsteps)),
    main = "Influence Propagation")
