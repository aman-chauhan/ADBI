...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    pos_limit = 0.01 * len(train_pos)
    neg_limit = 0.01 * len(train_neg)
    words = dict()
    for x in train_pos:
        for word in set(x) - stopwords:
            if word not in words:
                words[word] = {'pos': 0, 'neg': 0}
            words[word]['pos'] += 1
    for x in train_neg:
        for word in set(x) - stopwords:
            if word not in words:
    ...
    tmp = list(words.keys())
    for word in tmp:
        condition = (words[word]['pos'] >= pos_limit or words[word]['neg'] >= neg_limit)
        condition = condition and (words[word]['pos'] >= 2 * words[word]['neg']
                                   or words[word]['neg'] >= 2 * words[word]['pos'])
        if not condition:
            words.pop(word)
    ...
    for i in range(len(train_pos)):
        train_pos_vec.append([1 if x in train_pos[i] else 0 for x in features])
    for i in range(len(train_neg)):
        train_neg_vec.append([1 if x in train_neg[i] else 0 for x in features])
    for i in range(len(test_pos)):
        test_pos_vec.append([1 if x in test_pos[i] else 0 for x in features])
    for i in range(len(test_neg)):
        test_neg_vec.append([1 if x in test_neg[i] else 0 for x in features])
...
def build_models_NLP(train_pos_vec, train_neg_vec):
    ...
    nb_model = sklearn.naive_bayes.BernoulliNB(alpha=1.0, binarize=None).fit(train_pos_vec + train_neg_vec, Y)
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    tn, fp, fn, tp = confusion_matrix(["pos"] * len(test_pos_vec) + ["neg"] * len(test_neg_vec),
                                      model.predict(test_pos_vec + test_neg_vec)).ravel()
    accuracy = accuracy_score(["pos"] * len(test_pos_vec) + ["neg"] * len(test_neg_vec),
                              model.predict(test_pos_vec + test_neg_vec))
    if print_confusion:
