...
def dist(wt, X, Xi):
    selector_X = X.index != u'Yi'
    selector_Xi = Xi.index != u'Yi'
    mean_X = X.loc[selector_X].mean()
    mean_Xi = Xi.loc[selector_Xi].mean()
    std_X = X.loc[selector_X].std(ddof=0)
    std_Xi = Xi.loc[selector_Xi].std(ddof=0)
    return wt * (((X.loc[selector_X] - mean_X) / std_X) * ((Xi.loc[selector_Xi] - mean_Xi) / std_Xi)).sum() / Xi.loc[selector_Xi].size
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0, len(Xi.index)):
        sigma_YiXXi += Xi.iloc[i, -1] * math.exp(dist(wt, X, Xi.iloc[i]))
        sigma_XXi += math.exp(dist(wt, X, Xi.iloc[i]))
    return sigma_YiXXi / sigma_XXi
