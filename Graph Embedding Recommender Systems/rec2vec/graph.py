...
class Node(object):
    ...
    def __repr__(self):
        return '{} | {} | [{}] | {}'.format(self.id, self.name,
                                            ', '.join([x.id for x in self.neighbors]), self.type)
...
def records_to_graph():
    ...
    # create user nodes
    map(lambda user: (nodelist.append(user), nodedict.update({user.name: user})),
        map(lambda x: Node('node:{}'.format(x), x, 'user'), ratings.keys()))

    # create movie nodes
    map(lambda movie: (nodelist.append(movie), nodedict.update({movie.name: movie})),
        map(lambda x: Node('node:{}'.format(x), x, 'movie'), movies.keys()))

    # create director nodes
    map(lambda director: (nodelist.append(director), nodedict.update({director.name: director})),
        map(lambda x: Node('node:{}'.format(x), x, 'director'), directors))

    # create actor nodes
    map(lambda actor: (nodelist.append(actor), nodedict.update({actor.name: actor})),
        map(lambda x: Node('node:{}'.format(x), x, 'actor'), actors))

    # create genre nodes
    map(lambda genre: (nodelist.append(genre), nodedict.update({genre.name: genre})),
        map(lambda x: Node('node:{}'.format(x), x, 'genre'), genres))

    # create rating nodes
    map(lambda movie_ratings: (nodelist.extend(movie_ratings),
                               nodedict.update({node.name: node for node in movie_ratings})),
        map(lambda x: [Node('node:{}_{}'.format(x, i),
                            '{}_{}'.format(x, i),
                            'rating') for i in range(1, 6)], movies.keys()))

    for key in movies.keys():
        if movies[key].director is not None:
            nodedict[key].neighbors.append(nodedict[movies[key].director])
            nodedict[movies[key].director].neighbors.append(nodedict[key])
        for i in range(1, 6):
            nodedict['{}_{}'.format(key, i)].neighbors.append(nodedict[key])
            nodedict[key].neighbors.append(nodedict['{}_{}'.format(key, i)])
    ...
    for user in ratings.keys():
        for movie, rating in ratings[user].iteritems():
            nodedict[user].neighbors.append(nodedict['{}_{}'.format(movie, rating)])
            nodedict['{}_{}'.format(movie, rating)].neighbors.append(nodedict[user])
